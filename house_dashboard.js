/**
 * @file house_dashboard.js
 * @description Browser-based dashboard for the House Edge Tracker
 * 
 * This script sets up a web server that displays the House Edge Tracker data
 * in a browser window. It runs alongside the tracker_only.js script and
 * displays the same data in a more flexible, resizable interface.
 */
...
class DashboardHouseTracker extends HouseTracker {
  ...
  // Override updateUIDisplay with empty function
  updateUIDisplay() {
    // Do nothing - we'll use emitState instead
  }
  
  // Create a stats summary for display
  getStatsSummary() {
    const stats = {
      gameCount: this.gameStats.length,
      rugPullCount: this.gameStats.filter(g => g.rugPull).length,
      buySignals: {
      ...
      recentGames: this.gameResults.slice(-5).reverse()
    };
    
    return stats;
  }
  
  // Add a method to reset stats via dashboard
  resetStatsFromDashboard() {
    this.resetStats();
    io.emit('stats-reset');
    logger.info('[Dashboard] Statistics Reset from Dashboard');
  }
}
...
<body>
  ...
  <script>
    ...
    socket.on('tracker-update', (state) => {
      ...
      rugRiskContainer.innerHTML = '';
      if (state.maxLiability > 0.05) {
      ...
      if (state.activePrediction) {
        ...
        predHtml += 'Active Prediction: ' + pred.type.toUpperCase() + ' @ ' + pred.entryPrice.toFixed(3) + 'x (Tick ' + pred.entryTick + ')';
        if (pred.type === 'buy') {
          predHtml += ' Target: ≥' + pred.targetPriceUp.toFixed(3) + 'x';
        } else {
          predHtml += ' Target: ≤' + pred.targetPriceDown.toFixed(3) + 'x';
        }
        predHtml += '</div>';
...
function displayStatsAndExit(exitCode = 0) {
  try {
    logger.info('------------------------------------');
    logger.info('      DASHBOARD SESSION STATS       ');
    logger.info('------------------------------------');
...
(async () => {
  try {
    ...
    logger.info('[Dashboard] House Edge Tracker initialized.');

    const pages = await browser.pages();
    const page = pages.length > 0 ? pages[0] : await browser.newPage();
    logger.info(`[Dashboard] Navigating to: ${URL}`);
    
    // Set window size for better viewing
    await page.setViewport({ width: 1280, height: 800 });
    
    await page.goto(URL, { waitUntil: 'networkidle2' });
    logger.info('[Dashboard] Page navigated successfully.');
    await wait(1000);

    const ws = await setupWebSocketListener(page);
    if (!ws) throw new Error('Failed to set up WebSocket listener.');
    logger.info('[Dashboard] WebSocket listener attached. Tracker running...');
    
    // Process WebSocket messages
    ws.on('message', (parsedData) => {
      // Pass message to HouseTracker
      if (houseTracker) {
        houseTracker.processMessage(parsedData);
      }

      if (parsedData?.type !== 'gameStateUpdate') {
        logger.debug(`[Dashboard] WS Msg: ${parsedData?.type || 'N/A'}`);
      }
    });

    ws.on('close', () => {
      logger.error('[Dashboard] WebSocket connection closed unexpectedly.');
      displayStatsAndExit(1); 
    });

    logger.info('[Dashboard] House Edge Tracker running with browser dashboard.');
    logger.info('[Dashboard] Open http://localhost:3000 in your browser to view the dashboard.');
